...
protected:
	...
	{	// Thnx to this function, the "Helper" can see that we are "extended" logger !!!
		...
		DWORD cbWritten=0;
		char *ptr=const_cast<char*>("Extended-Console: TRUE\r\n");
...
/* 패킷캡쳐클래스*/
// 네트워크 목록을 Linked List 로 받아옴
// 네트워크 선택 후 열기
// 핸들에 저장 후 Linked List 폐기
// 함수 넘겨서 패킷정보 출력( Second Console )
...
private:
	...
	// Linked lisk of Network Device
	pcap_if_t* Devices;
	pcap_if_t* tempDevices;
	pcap_t* handle;

	// Device count
	int DeviceIndex;

	// 패킷 캡쳐가 시작될때 호출 될 콜백함수
	// param : 파라미터로 받은 값
	// header : 패킷 헤더 정보
	// pkt_data : 캡쳐된 데이터
	CConsoleLogger PacketConsole;
	void Handler(u_char* param, const struct pcap_pkthdr* header, const u_char* pkt_data);

public:

	PktCapRetrunType OpenNetworkDevice(int select);
	PktCapRetrunType ShowNetworkDeviceList();

	PacketCapture() {
		memset(this->errBuff, 0, PCAP_ERRBUF_SIZE);
		PacketConsole.Create("Packet Console");
